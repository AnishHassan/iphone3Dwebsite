...
const VideoCarousel = () => {
    ...
    const handleLooadedMetaData = (i, e) => setLoadedData((pre) => [...pre, e])
                    ...
                    <div key={list.id} id='slider' className='sm:pr-20 pr-10'>
                        <div className='video-carousel_container'>
                            <div className='w-full h-full flex-center rounded-3xl overflow-hidden bg-black '>
                                <video id='video' autoPlay playsInline={true} preload='auto' muted ref={(el) => (videoRef.current[i] = el)} onPlay={() => {
                                    setVideo((prevVideo) => ({
                                        ...prevVideo, isPlaying: true
                                    }))
                                }} onLoadedMetadata={(e) => handleLooadedMetaData(i, e)} onEnded={() => i !== 3
            ...
            <div className='relative flex-center mt-10'>
                <div className='flex-center py-5 px-7 bg-gray-500 backdrop-blur rounded-full '>
                    {videoRef.current.map((_, i) => (
                        <span key={i} ref={(el) => (videoDevRef.current[i] = el)} className='mx-2 w-3 h-3 bg-gray-100 rounded-full relative cursor-pointer'>
